...
buildMetaLink(BuildContext context, Aria2ServerConfig config) {
  return FormGroup(
      ...
      items: [
        ...
        FormItem<bool>(
            label: '下载 Metalink 中的文件',
            type: FormItemType.switch_,
            value: config.followMetaLink,
            key: 'follow-metalink',
            tooltip:
                "如果设置为\"是\"或\"仅内存\", 当后缀为 .meta4 或 .metalink 或内容类型为 application/metalink4+xml 或 application/metalink+xml 的文件下载完成时, aria2 将按Metalink 文件读取并下载该文件中提到的文件. 如果设置为\"仅内存\", 该 Metalink 文件将不会写入到磁盘中, 而仅会存储在内存中. 如果设置为\"否\", 则 .metalink 文件会下载到磁盘中, 但不会按 Metalink 文件读取并且其中的文件不会进行下载."),
        ...
        FormItem<String>(
            ...
            tooltip: "首选服务器所在的位置. 可以使用逗号分隔的列表, 例如: jp,us."),
